...
class MyClass(ui.View):
	...
	def fn_convert(self, sender):
		...
		if tf_carb_per_100.text == '':
			...
			# Меняем запятую в числе на точку
			tf_param_to.text = tf_param_to.text.replace(',','.')
			tf_carb_per_100.text = tf_carb_per_100.text.replace(',','.')
			
			# Расчет
			# Сколько грамм продукта даем
			if sc_option.selected_index == 0:

				amount = float(tf_param_to.text) * float(tf_carb_per_100.text) / 100 / 12

				ans = f'{round(amount,2)} ХЕ в {tf_param_to.text} грамм(а) продукта'

			# Сколько ХЕ нужно дать
			elif sc_option.selected_index == 1:

				amount = float(tf_param_to.text) * 12 * 100 / float(tf_carb_per_100.text)

				ans = f'{round(amount,2)} грамм(а) продукта даём на {tf_param_to.text} ХЕ'
...
# Зауск
##################
v = MyClass(name='Калькулятор углеводов')
